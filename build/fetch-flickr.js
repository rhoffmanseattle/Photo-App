...
async function main() {
  ...
  // 11. Ship redirect rules inside the publish dir. netlify.toml only
  // applies when Netlify builds from git; a manual deploy of dist/
  // (drag-drop or CLI) doesn't include it, which silently drops the
  // /img/* Flickr proxy and breaks every image. A _redirects file in
  // the publish dir applies in every deploy mode, so emit one with the
  // same rules as netlify.toml.
  const redirects = [
    "# Generated by build/fetch-flickr.js — keep in sync with netlify.toml",
    "/img/*  https://live.staticflickr.com/:splat  200",
    "/p/:id  /p/:id/index.html  200",
    "/c/:slug  /c/:slug/index.html  200",
    "/g/:slug  /g/:slug/index.html  200",
  ].join("\n");
  await fs.writeFile(path.join(DIST, "_redirects"), redirects + "\n");
  console.log("[write] dist/_redirects");
